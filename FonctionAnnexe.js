    ...
    parsedCourses.forEach(course => {
        if (selectedCourseCodes.includes(course.courseCode)) {
            course.timeSlots.forEach(slot => {
                ...
                const dayMapping = {
                    "L": 1, "MA": 2, "ME": 3, "J": 4, "V": 5, "S": 6
                };
                console.log(slot.horaire);
                ...
                while (currentDate <= endDate) {
                    if (currentDate.getDay() === dayOfWeek) {
                        events.push({
                            ...
                            start: new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), ...startTime.split(":").map(Number)),
                            end: new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), ...endTime.split(":").map(Number)),
                            location: slot.salle,
                            description: `Type: ${slot.type}`
                        });
                    }
